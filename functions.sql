...
BEGIN
  ...
  folder_results AS (
    ...
    GROUP BY af.id, af.name, af.is_group, af.created_at, af.user_id
  ),
  group_results AS (
    SELECT
    ...
    GROUP BY ag.id, ag.name, ag.is_group, ag.created_at, ag.user_id
  ),
  link_results AS (
    SELECT
