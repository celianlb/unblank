...
END;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| public      | check_edit_permission_plan                 |                                                                                                                                                                                       | trigger                                                                                                                                                                                                                                                                                                                   | VOLATILE   | FUNCTION      | plpgsql  | 
DECLARE
  folder_owner_id uuid;
  owner_plan text;
BEGIN
  -- Si la permission n'est pas 'edit', autoriser
  IF NEW.permission != 'edit' THEN
    RETURN NEW;
  END IF;
  ...
  SELECT subscription_plan INTO owner_plan
  FROM public.users
  WHERE id = folder_owner_id;

  -- Vérifier que le propriétaire a un plan Pro ou Team
  IF owner_plan IS NULL OR owner_plan = 'free' THEN
    RAISE EXCEPTION 'Edit permission requires Pro or Team plan. Please upgrade to share with edit permissions.'
...
BEGIN
  ...
  SELECT 
    ...
    t.name,
    t.created_at,
    COUNT(DISTINCT lt.id)::integer as usage_count,
    COUNT(DISTINCT lt.link_id)::integer as link_count,
    MAX(lt.created_at) as last_used_at
  FROM public.tags t
  LEFT JOIN public.link_tags lt ON t.id = lt.tag_id
  LEFT JOIN public.links l ON lt.link_id = l.id
  WHERE (
    -- Tags appartenant à l'utilisateur
    t.user_id = p_user_id
    OR
    -- Tags accessibles via liens partagés
    EXISTS (
      SELECT 1
      FROM public.link_tags lt2
      JOIN public.links l2 ON lt2.link_id = l2.id
      JOIN public.shares s ON s.folder_id = l2.folder_id
      WHERE lt2.tag_id = t.id
        AND s.is_active = true
        AND (s.expires_at IS NULL OR s.expires_at > now())
        AND (s.share_token IS NOT NULL OR s.shared_with_email = (SELECT auth.jwt()->>'email'))
  ...
  -- Filtrer pour inclure seulement les tags sur des liens accessibles
  AND (
    l.user_id = p_user_id
    OR
    EXISTS (
  ...
  GROUP BY t.id, t.user_id, t.name, t.created_at
  HAVING COUNT(DISTINCT lt.id) > 0 OR t.user_id = p_user_id
  ORDER BY usage_count DESC, t.name ASC;
END;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
END;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| public      | reset_monthly_links                        |                                                                                                                                                                                       | void                                                                                                                                                                                                                                                                                                                      | VOLATILE   | FUNCTION      | plpgsql  | BEGIN
  UPDATE public.users
  SET 
...
BEGIN
  ...
  IF p_tag_names IS NOT NULL AND array_length(p_tag_names, 1) > 0 THEN
    ...
    WHERE (
      -- Liens appartenant à l'utilisateur
      l.user_id = p_user_id
      OR
      -- Liens partagés via folders (view ou edit)
      EXISTS (
        SELECT 1
        FROM public.shares s
        WHERE s.folder_id = l.folder_id
...
END;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| public      | unified_search                             | p_user_id uuid, p_query text DEFAULT NULL::text, p_tag_names text[] DEFAULT NULL::text[], p_limit integer DEFAULT 20, p_offset integer DEFAULT 0                                      | TABLE(result_type text, id uuid, name text, url text, description text, screenshot_url text, folder_id uuid, folder_name text, is_group boolean, link_count integer, tags json, created_at timestamp with time zone, is_shared boolean, slug text, parent_folder_id uuid, parent_slug text)                               | VOLATILE   | FUNCTION      | plpgsql  | 
BEGIN
  RETURN QUERY
  WITH accessible_folders AS (
    -- Dossiers/groupes accessibles par l'utilisateur
    SELECT 
      f.id, 
      f.name, 
      f.is_group, 
  ...
  accessible_links AS (
    ...
    AND (
      p_tag_names IS NULL 
      OR array_length(p_tag_names, 1) = 0
      OR l.id IN (
        SELECT lt2.link_id
        FROM public.link_tags lt2
        JOIN public.tags t2 ON lt2.tag_id = t2.id
  ...
  folder_results AS (
    SELECT
      ...
      '[]'::json as tags,
      af.created_at,
      (af.user_id != p_user_id) as is_shared,
      af.slug,
      af.parent_folder_id,
      parent_group.slug as parent_slug
    FROM accessible_folders af
    LEFT JOIN public.links l ON l.folder_id = af.id
    LEFT JOIN public.folders parent_group ON af.parent_folder_id = parent_group.id
    WHERE af.is_group = false
    GROUP BY af.id, af.name, af.is_group, af.created_at, af.user_id, af.slug, af.parent_folder_id, parent_group.slug
  ),
  group_results AS (
    SELECT
      ...
      '[]'::json as tags,
      ag.created_at,
      (ag.user_id != p_user_id) as is_shared,
      ag.slug,
      NULL::uuid as parent_folder_id,
      NULL::text as parent_slug
    FROM accessible_folders ag
    LEFT JOIN public.folders sub ON sub.parent_folder_id = ag.id
    LEFT JOIN public.links l ON l.folder_id = sub.id
    WHERE ag.is_group = true
    GROUP BY ag.id, ag.name, ag.is_group, ag.created_at, ag.user_id, ag.slug
  ),
  link_results AS (
    SELECT
      'link'::text as result_type,
      al.id,
      COALESCE(al.title, al.url) as name,
      al.url,
      al.description,
      COALESCE(al.original_image_url, al.screenshot_url) as screenshot_url,
      al.folder_id,
      al.folder_name,
      false as is_group,
      0 as link_count,
      COALESCE(
        (
          ...
          FROM public.link_tags lt
          JOIN public.tags t ON lt.tag_id = t.id
          WHERE lt.link_id = al.id
        ),
        '[]'::json
      ) as tags,
      al.created_at,
